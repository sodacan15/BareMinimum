...
# Mapping for displayimport streamlit as st
from datetime import time
import sys
sys.path.append('.')
...
def render_task(task, day_obj, current_day):
    ...
    with st.expander(f"📋 {task.taskName}", expanded=False):
        ...
        if available_days and len(recurrence_days) < 7:
            ...
            with c2:
                if st.button("➕", key=f"btn_add_{task_key}_{current_day}_{id(task)}"):
                    if new_day != '...':
        ...
        for idx, day in enumerate(recurrence_days):
            with tabs[idx]:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    hard = st.checkbox("Hard", task.taskDeadline == 1, key=f"h_{task_key}_{day}_{current_day}_{id(task)}")
                    task.setValue("taskDeadline", 1 if hard else 0)
                with c2:
                    st.checkbox("Soft", task.taskDeadline == 0, key=f"s_{task_key}_{day}_{current_day}_{id(task)}", disabled=True)
                
                # TIME
                c1, c2, c3 = st.columns(3)
                with c1:
                    tin = st.time_input("In", mins_to_time(task.timeStart), key=f"ti_{task_key}_{day}_{current_day}_{id(task)}")
                    task.setValue("timeStart", time_to_mins(tin))
                with c2:
                    tout = st.time_input("Out", mins_to_time(task.timeEnd), key=f"to_{task_key}_{day}_{current_day}_{id(task)}")
                ...
                for i, sub in enumerate(task.subTasks):
                    ...
                    with c2:
                        ...
                        if new != sub.name:
                            sub.name = new
                    with c3:
                        if st.button("🗑", key=f"ds_{task_key}_{day}_{current_day}_{i}_{id(task)}"):
                            to_remove.append(sub)
                
                for sub in to_remove:
                    task.removeSubTask(sub)
                
                if st.button("➕ Subtask", key=f"as_{task_key}_{day}_{current_day}_{id(task)}"):
        ...
        st.divider()
        if st.button(f"🗑 Delete All", key=f"del_{task_key}_{current_day}_{id(task)}"):
